...
AddEventHandler("Proxy:Shared:RegisterReady", function()
	exports["pulsar_core"]:RegisterComponent("Minigame", MINIGAME)
end)
...
function _playGame(params, action)
	...
	if not IsEntityDead(player) or action.useWhileDead then
		if not isPlayingGame then
			...
			plsr.Notification:Error("Already Doing An Action", 5000)
		end
	else
		plsr.Notification:Error("Already Doing An Action", 5000)
	end
				...
				if not isAnim then
					if action.animation then
						...
						elseif action.animation.anim ~= nil then
							plsr.Animations.Emotes:Play(action.animation.anim, false, action.duration or -1, true)
						else
							if GetVehiclePedIsIn(PlayerPedId()) == 0 then
...
function _gameCleanup(action)
	...
	if action ~= nil and action.animation then
		...
		elseif action.animation.anim ~= nil then
			...
			if action.animation.task ~= nil and GetVehiclePedIsIn(PlayerPedId()) == 0 then
				ClearPedTasksImmediately(PlayerPedId())
			end
		end
	end
